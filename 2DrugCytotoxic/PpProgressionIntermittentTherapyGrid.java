...
public class PpProgressionIntermittentTherapyGrid extends AgentGrid2D<PpProgressionIntermittentTherapyCell> {
    //model constants
...
//    public double DRUG_ON_TIME = 40, DRUG_OFF_TIME = 160, DRUG_CYCLE_TIME = 200, DRUG_DIFF_RATE = 2.0, DRUG_UPTAKE = 0.91, DRUG_BOUNDARY_VAL = 1.0, DRUG_METABOLISM_TIME = 40.0, DRUG_A_DEATH=0.2, DRUG_B_DEATH=0.2;
    ...
    public static void main(String[] args) {
        ...
        PpProgressionIntermittentTherapyGrid[] models = new PpProgressionIntermittentTherapyGrid[4];
        String strategy = "Int";
        String sub_strategy = "PpProgLoDrug";
        String file_counter = "Test1" ;
...
//
//        ShuffleAgents(rn);
//        for (PpProgressionIntermittentTherapyCell cell : this) {
//            cell.CellStep();
//        }
//
...
//                    break;
        ...
        for (int i = 0; i < length; i++) {
            PpProgressionIntermittentTherapyCell drawMe = GetAgent(i);
            ...
            if ((tick-AT_START_TICK)%CHECK_TUMORSIZE_INTERVAL_AT==0) {
                ...
                if(IS_IT_DOWN_PHASE) {
                    if (IS_LAST_DRUG_DELIVERED_A) {
                        if (CURRENT_POP>0.5*TUMOR_SIZE_TRIGGERING_AT && CURRENT_POP<TUMOR_SIZE_TRIGGERING_AT) {
                            CURRENT_DRUG_A=MAX_TOLERATED_DOSE_A;
                            CURRENT_DRUG_B=0.0;
                            System.out.println("Tick: "+tick+" Current Drug for CurrentPop "+CURRENT_POP+"is "+CURRENT_DRUG_A+" and"+CURRENT_DRUG_B+" respectively");
                            IS_LAST_DRUG_DELIVERED_B=false;
...
class PpProgressionIntermittentTherapyCell extends AgentSQ2Dunstackable<PpProgressionIntermittentTherapyGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        PpProgressionIntermittentTherapyCell tmp = G.NewAgentSQ(index);
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                PpProgressionIntermittentTherapyCell tmpAgent = G.GetAgent(tmpAgentIndex);
