...
public class PpAlternatePenultimateAddGrid extends AgentGrid2D<PpAlternatePenultimateAddCell> {
    ...
    public double AT_ALPHA_DRUG_A, AT_ALPHA_DRUG_B,AT_GAMMA_DRUG_A,AT_GAMMA_DRUG_B, AT_BETA, AT_BETA_X, AT_BETA_Y, MAX_TOLERATED_DOSE_A, MAX_TOLERATED_DOSE_B, MIN_DRUG_DOSE_A, MIN_DRUG_DOSE_B;
    ...
    public PpAlternatePenultimateAddGrid(int xDim, int yDim, Rand rn, double birthSensitiveProb, double birthResistantProb, double deathProb, double AT_Alpha_Drug_A, double AT_Alpha_Drug_B, double AT_Gamma_Drug_A,double AT_Gamma_Drug_B, double AT_Beta, double AT_Beta_X, double AT_Beta_Y, double Max_Tolerated_dose_A, double Max_Tolerated_dose_B, double Min_Drug_Dose_A, double Min_Drug_Dose_B, double DRUG_ON_TIME, double DRUG_CYCLE_TIME, double DRUG_DIFF_RATE, double DRUG_UPTAKE, double DRUG_A_DEATH, double DRUG_B_DEATH, int check_tumorsize_interval_AT, double tumor_size_percent_triggering_AT, double replacement_threshold, double measurement_noise_sd,UILabel tickLabel,UILabel popLabel,UILabel drugLabel) {
        ...
        this.AT_BETA_X = AT_Beta_X;
        this.AT_BETA_Y = AT_Beta_Y;
...
//                    }
                    ...
                    if (PREV_POP_DRUG_A > ((int) (((1 + (AT_BETA+AT_BETA_X)) * PREV_POP_DRUG_B)))) {
...
//                        CURRENT_POP_DRUG_A=CURRENT_POP;
                    ...
                    if (PREV_POP_DRUG_A <= ((int) (((1 - (AT_BETA+AT_BETA_Y)) * PREV_POP_DRUG_B)))) {
...
//                        CURRENT_POP_DRUG_A=CURRENT_POP;
                    ...
                    if (PREV_POP_DRUG_A > ((int) ((1 - (AT_BETA+AT_BETA_Y)) * PREV_POP_DRUG_B)) && PREV_POP_DRUG_A <= ((int) ((1 + (AT_BETA+AT_BETA_X)) * PREV_POP_DRUG_B))) {
...
//                        return true;
//                    }
                    if (PREV_POP_DRUG_B > ((int) (((1 + (AT_BETA+AT_BETA_X)) * PREV_POP_DRUG_A)))) {
...
//                        CURRENT_POP_DRUG_B=CURRENT_POP;
                    ...
                    if (PREV_POP_DRUG_B <= ((int) (((1 - (AT_BETA+AT_BETA_Y)) * PREV_POP_DRUG_A)))) {
...
//                        CURRENT_POP_DRUG_B=CURRENT_POP;
                    ...
                    if (PREV_POP_DRUG_B > ((int) ((1 - (AT_BETA+AT_BETA_Y)) * PREV_POP_DRUG_A)) && PREV_POP_DRUG_B <= ((int) ((1 + (AT_BETA+AT_BETA_X)) * PREV_POP_DRUG_A))) {
                        if (VARIABLE_BETA > 0 && CURRENT_POP < INITIAL_POP) {
...
//        Mutate();
        ...
        if(type==G.RESISTANT_TO_DRUG_A && tmp_random<G.DEATH_PROB) {
            Dispose();
            G.RES_DRUG_A_POP--;
            return;
        }
        if(type==G.RESISTANT_TO_DRUG_B && tmp_random<G.DEATH_PROB) {
            Dispose();
            G.RES_DRUG_B_POP--;
            return;
        }
        ...
        double tmp_div_prob;
        if (this.type==G.DOUBLY_SENSITIVE) {
            tmp_div_prob=G.DIV_PROB_SEN-(G.drugA.Get(Isq())*G.DRUG_A_DEATH+G.drugB.Get(Isq())*G.DRUG_B_DEATH);
        }
        else if (this.type==G.DOUBLY_RESISTANT) {
            tmp_div_prob=G.DIV_PROB_RES;
        }
        else if (this.type==G.RESISTANT_TO_DRUG_A) {
            tmp_div_prob=G.DIV_PROB_RES_DRUG_A-G.drugB.Get(Isq())*G.DRUG_B_DEATH;
        }
        else if (this.type==G.RESISTANT_TO_DRUG_B) {
            tmp_div_prob=G.DIV_PROB_RES_DRUG_B-G.drugA.Get(Isq())*G.DRUG_A_DEATH;
        } else {
