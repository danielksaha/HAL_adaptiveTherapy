...
public class OneDrugCytotoxicDoseAdjustmentGrid extends AgentGrid2D<OneDrugCytotoxicDoseAdjustmentCell> {
    //model constants
...
//        drugB = new PDEGrid2D(xDim, yDim);
    ...
    public static void main(String[] args) {
        ...
        OneDrugCytotoxicDoseAdjustmentGrid[] models = new OneDrugCytotoxicDoseAdjustmentGrid[4];
        String strategy = "DosAdjAdd50";
        String sub_strategy ="1DrugCytotoxic";
        String file_counter ="3";
...
//        double AT_gamma_drug_B=0;
        double AT_beta=0.1;
        double Max_Tolerated_Dose_A=2.5;
...
//
//        ShuffleAgents(rn);
//        for (OneDrugCytotoxicDoseAdjustmentCell cell : this) {
//            cell.CellStep();
//        }
//
...
//                    break;
        ...
        for (int i = 0; i < length; i++) {
            OneDrugCytotoxicDoseAdjustmentCell drawMe = GetAgent(i);
...
class OneDrugCytotoxicDoseAdjustmentCell extends AgentSQ2Dunstackable<OneDrugCytotoxicDoseAdjustmentGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        OneDrugCytotoxicDoseAdjustmentCell tmp = G.NewAgentSQ(index);
...
//        G.drugB.Mul(Isq(), G.DRUG_UPTAKE);
        ...
        if(type==G.DOUBLY_SENSITIVE && tmp_random<(G.DEATH_PROB+G.drugA.Get(Isq())*G.DRUG_A_DEATH)) {
            Dispose();
            G.SEN_POP--;
            return;
        }
...
//            tmp_div_prob=G.DIV_PROB_RES_DRUG_B;
        ...
        if (G.rn.Double() < tmp_div_prob) {
            ...
            if (options_empty==0 && G.rn.Double()<G.REPLACEMENT_THRESHOLD) { //&& this.type==G.DOUBLY_RESISTANT) {
                ...
                OneDrugCytotoxicDoseAdjustmentCell tmpAgent = G.GetAgent(tmpAgentIndex);
