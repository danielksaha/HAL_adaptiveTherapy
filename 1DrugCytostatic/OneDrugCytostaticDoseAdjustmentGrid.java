...
public class OneDrugCytostaticDoseAdjustmentGrid extends AgentGrid2D<OneDrugCytostaticDoseAdjustmentCell> {
    //model constants
    ...
    public final static int DOUBLY_RESISTANT = CategorialColor(1), DOUBLY_SENSITIVE = CategorialColor(2);
    public final static double MEASUREMENT_NOISE_MULTIPLIER=0.5;
    public double DIV_PROB_SEN, DIV_PROB_RES, DEATH_PROB, PREV_DRUG_A, CURRENT_DRUG_A;
...
//    public double DRUG_ON_TIME = 40, DRUG_OFF_TIME = 160, DRUG_CYCLE_TIME = 200, DRUG_DIFF_RATE = 2.0, DRUG_UPTAKE = 0.91, DRUG_BOUNDARY_VAL = 1.0, DRUG_METABOLISM_TIME = 40.0, DRUG_A_DEATH=0.2, DRUG_B_DEATH=0.2;
    public double DRUG_ON_TIME, DRUG_CYCLE_TIME, DRUG_DIFF_RATE, DRUG_UPTAKE, DRUG_A_DEATH;

    public double AT_ALPHA_DRUG_A, AT_GAMMA_DRUG_A,AT_BETA, MAX_TOLERATED_DOSE_A, MIN_DRUG_DOSE_A;
...
//    public double Mut_ResB_to_DoublySen = 1e-3;
    ...
    public OneDrugCytostaticDoseAdjustmentGrid(int xDim, int yDim, Rand rn, double birthSensitiveProb, double birthResistantProb, double deathProb, double AT_Alpha_Drug_A,double AT_Gamma_Drug_A,double AT_Beta, double Max_Tolerated_dose_A,double Min_Drug_Dose_A, double DRUG_ON_TIME, double DRUG_CYCLE_TIME ,double DRUG_DIFF_RATE,double DRUG_UPTAKE,double DRUG_A_DEATH,int check_tumorsize_interval_AT, double tumor_size_percent_triggering_AT, double replacement_threshold, double measurement_noise_sd,UILabel tickLabel,UILabel popLabel,UILabel drugLabel) {
        super(xDim, yDim, OneDrugCytostaticDoseAdjustmentCell.class);
        this.rn = rn;
...
//        drugB = new PDEGrid2D(xDim, yDim);
    ...
    public static void main(String[] args) {
        ...
        OneDrugCytostaticDoseAdjustmentGrid[] models = new OneDrugCytostaticDoseAdjustmentGrid[4];
        String strategy = "DosAdjAdd50";
        String sub_strategy ="1DrugCytostatic";
...
//            models[i]=new OneDrugCytostaticDoseAdjustmentGrid(x,y,new Rand(1),Div_Prob_Sen,Div_Prob_Res,deathProb,AT_alpha_drug_A,AT_alpha_drug_B,AT_gamma_drug_A,AT_gamma_drug_B,AT_beta,Max_Tolerated_Dose_A,Max_Tolerated_Dose_B, Min_Drug_Dose_A,Min_Drug_Dose_B,DRUG_ON_TIME,DRUG_CYCLE_TIME,DRUG_DIFF_RATE,DRUG_UPTAKE,DRUG_A_DEATH,DRUG_B_DEATH,Check_Tumorsize_Interval_AT,Tumor_Size_Percent_Triggering_AT,Replacement_Threshold,Measurement_Noise_SD);
            models[i]=new OneDrugCytostaticDoseAdjustmentGrid(x,y,new Rand(System.currentTimeMillis()),Div_Prob_Sen,Div_Prob_Res,deathProb,AT_alpha_drug_A,AT_gamma_drug_A,AT_beta,Max_Tolerated_Dose_A, Min_Drug_Dose_A,DRUG_ON_TIME,DRUG_CYCLE_TIME,DRUG_DIFF_RATE,DRUG_UPTAKE,DRUG_A_DEATH,Check_Tumorsize_Interval_AT,Tumor_Size_Percent_Triggering_AT,Replacement_Threshold,Measurement_Noise_SD,tickLabel,popLabel,drugLabel);
...
//        models[2].DRUG_CYCLE_TIME = 200;
        ...
        for (int tick = 0; tick < 5000; tick++) {
            ...
            //data recording
            popsOut.Write(tick+","+strategy+","+sub_strategy+","+models[0].Pop()+","+models[0].SEN_POP+","+models[0].RES_POP+","+models[1].Pop()+","+models[1].SEN_POP+","+models[1].RES_POP+","+models[2].Pop()+","+models[2].SEN_POP+","+models[2].RES_POP+","+models[3].Pop()+","+models[3].SEN_POP+","+models[3].RES_POP+","+models[3].CURRENT_DRUG_A+",");
...
//
//        ShuffleAgents(rn);
//        for (OneDrugCytostaticDoseAdjustmentCell cell : this) {
//            cell.CellStep();
//        }
//
...
//    }
    public void ModelStep(int tick) {
        ...
        if (HAS_STANDARD_THERAPY_STARTED) {
            ...
            if(DRUG_CYCLE_ITERATOR>=DRUG_CYCLE_TIME) {
                Drug_Deliverer(DRUG_DIFF_RATE,MAX_TOLERATED_DOSE_A,false);
                DRUG_CYCLE_ITERATOR=0;
            }
                ...
                Drug_Deliverer(DRUG_DIFF_RATE,MAX_TOLERATED_DOSE_A,true);
                DRUG_CYCLE_ITERATOR++;
            }
        }
    ...
    public void ModelStepAdaptiveTherapy(int tick) {
        ...
        if (HAS_AT_STARTED) {
            ...
            if(DRUG_CYCLE_ITERATOR>=CHECK_TUMORSIZE_INTERVAL_AT) {
                Drug_Deliverer(DRUG_DIFF_RATE,CURRENT_DRUG_A,false);
                DRUG_CYCLE_ITERATOR=0;
            }
                ...
                Drug_Deliverer(DRUG_DIFF_RATE,CURRENT_DRUG_A,true);
                DRUG_CYCLE_ITERATOR++;
            }
        }
...
//    }
    ...
    public void Drug_Deliverer(double drug_diffusion_rate, double drugA_amount, boolean is_drug_on) {
...
//                    break;
                case 3:
                    ...
                    drugLabel.SetText("Current Drug (A) AT is "+CURRENT_DRUG_A+" "+"       Grid Avg is "+drugA.GetAvg());
                    break;
            }
        }
        for (int i = 0; i < length; i++) {
            OneDrugCytostaticDoseAdjustmentCell drawMe = GetAgent(i);
...
//        Start of Adaptive Therapy
        ...
        CURRENT_POP = Pop();
        CURRENT_POP = (int)rn.Gaussian(CURRENT_POP,MEASUREMENT_NOISE_SD);
//        CURRENT_POP = (int)rn.Gaussian(CURRENT_POP,MEASUREMENT_NOISE_MULTIPLIER*CURRENT_POP);
...
//                CURRENT_DRUG_B=MAX_TOLERATED_DOSE_B;
                System.out.println("Tick: "+tick+" Current Drug A for first dose and Current Pop "+CURRENT_POP+" >= Tumor size triggering AT is "+CURRENT_DRUG_A+" " +". Prev pop is "+PREV_POP);
                AT_START_TICK=tick;
        ...
        if (HAS_AT_STARTED) {
            ...
            if ((tick-AT_START_TICK)%CHECK_TUMORSIZE_INTERVAL_AT==0) {
                ...
                if(CURRENT_POP<=(0.5*TUMOR_SIZE_TRIGGERING_AT)) {
                    CURRENT_DRUG_A=0.0;
//                    CURRENT_DRUG_B=0.0;
                    System.out.println("Tick: "+tick+" Current Drug A for CurrentPop "+CURRENT_POP+" <=0.5 of the tumor size triggering AT is "+CURRENT_DRUG_A+" "+". Prev pop is "+PREV_POP);
                    PREV_POP=CURRENT_POP;
...
//                    CURRENT_DRUG_A=(PREV_DRUG_A+AT_GAMMA_DRUG_A)>=MAX_TOLERATED_DOSE_A?MAX_TOLERATED_DOSE_A:(PREV_DRUG_A+AT_GAMMA_DRUG_A);
...
//                    CURRENT_DRUG_B=(AT_GAMMA_DRUG_B*PREV_DRUG_B)>=MAX_TOLERATED_DOSE_B?MAX_TOLERATED_DOSE_B:((AT_GAMMA_DRUG_B)*PREV_DRUG_B);
                    System.out.println("Tick: "+tick+" Current Drug A for CurrentPop "+CURRENT_POP+" >(1+at_beta)*Prev Pop "+PREV_POP+" is "+CURRENT_DRUG_A);
                    PREV_POP=CURRENT_POP;
                    PREV_DRUG_A=CURRENT_DRUG_A;
//                    PREV_DRUG_B=CURRENT_DRUG_B;
                    IS_AT_ON=true;
...
class OneDrugCytostaticDoseAdjustmentCell extends AgentSQ2Dunstackable<OneDrugCytostaticDoseAdjustmentGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        OneDrugCytostaticDoseAdjustmentCell tmp = G.NewAgentSQ(index);
...
//        G.drugB.Mul(Isq(), G.DRUG_UPTAKE);
        ...
        if(type==G.DOUBLY_SENSITIVE && tmp_random<G.DEATH_PROB) {
            Dispose();
            G.SEN_POP--;
            return;
        }
...
//        }
        ...
        double tmp_div_prob;
        if (this.type==G.DOUBLY_SENSITIVE) {
            tmp_div_prob=G.DIV_PROB_SEN-(G.drugA.Get(Isq())*G.DRUG_A_DEATH);
        }
...
//            tmp_div_prob=G.DIV_PROB_RES_DRUG_B;
        ...
        if (G.rn.Double() < tmp_div_prob) {
            ...
            if (options_empty==0 && G.rn.Double()<G.REPLACEMENT_THRESHOLD) { //&& this.type==G.DOUBLY_RESISTANT) {
                ...
                OneDrugCytostaticDoseAdjustmentCell tmpAgent = G.GetAgent(tmpAgentIndex);
