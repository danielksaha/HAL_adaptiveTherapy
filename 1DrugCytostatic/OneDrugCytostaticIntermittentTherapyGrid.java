...
public class OneDrugCytostaticIntermittentTherapyGrid extends AgentGrid2D<OneDrugCytostaticIntermittentTherapyCell> {
    //model constants
...
//        drugB = new PDEGrid2D(xDim, yDim);
    ...
    public static void main(String[] args) {
        ...
        OneDrugCytostaticIntermittentTherapyGrid[] models = new OneDrugCytostaticIntermittentTherapyGrid[4];
        String strategy = "Int";
        String sub_strategy = "1DrugCytostaticLoDrug";
...
//
//        ShuffleAgents(rn);
//        for (OneDrugCytostaticIntermittentTherapyCell cell : this) {
//            cell.CellStep();
//        }
//
...
//                    break;
                case 3:
                    ...
                    drugLabel.SetText("Current Drugs (A and B) AT is "+CURRENT_DRUG_A+" "+"       Grid Avg is "+drugA.GetAvg());
                    break;
            }
        }
        for (int i = 0; i < length; i++) {
            OneDrugCytostaticIntermittentTherapyCell drawMe = GetAgent(i);
...
//                CURRENT_DRUG_B=MAX_TOLERATED_DOSE_B;
                System.out.println("Tick: "+tick+" Current Drug A for first dose and Current Pop "+CURRENT_POP+" >= Tumor size triggering AT is "+CURRENT_DRUG_A);
                AT_START_TICK=tick;
            ...
            if ((tick-AT_START_TICK)%CHECK_TUMORSIZE_INTERVAL_AT==0) {
                ...
                if(IS_IT_UP_PHASE) {
                    if (CURRENT_POP<TUMOR_SIZE_TRIGGERING_AT) {
                        CURRENT_DRUG_A=0.0;
//                        CURRENT_DRUG_B=0.0;
                        System.out.println("Tick: "+tick+" Current Drug for CurrentPop "+CURRENT_POP+"is "+CURRENT_DRUG_A);
                        return true;
                    }
                }
...
class OneDrugCytostaticIntermittentTherapyCell extends AgentSQ2Dunstackable<OneDrugCytostaticIntermittentTherapyGrid> {
...
//    public double drug_sensitivity_score;
    ...
    public void DivideAndMutate(int index) {
        OneDrugCytostaticIntermittentTherapyCell tmp = G.NewAgentSQ(index);
...
//                System.out.println("tmpAgentIndex is " + tmpAgentIndex);
                OneDrugCytostaticIntermittentTherapyCell tmpAgent = G.GetAgent(tmpAgentIndex);
